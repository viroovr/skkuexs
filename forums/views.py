...
SEARCH_ENGINE = "64304a3ea964f44ea"
...
def visa(request, school_name):
    ...
    update_date = Report.objects.filter(user_university=school_name).exclude(
        pre_visa_how__exact='').aggregate(Max('user_duration'))
    report = report_list[0]
    context = {
        'school_name': school_name,
...
def dorm(request, school_name):
    report_list = Report.objects.filter(user_university=school_name).annotate(
        count_now_dorm_name=Count('now_dorm_name', distinct=True)).order_by('-count_now_dorm_name')[:5]

    if not report_list:
        return render(request, 'forums/empty.html', {'school_name': school_name})

    webSite = Report.objects.filter(user_university=school_name).exclude(
        now_website__exact='').filter(now_website__startswith='http')
    if (webSite):
    ...
    update_date = Report.objects.filter(user_university=school_name).exclude(
        now_etc__exact='').aggregate(Max('user_duration'))
    report = report_list[0]
...
def etc_pre(request, school_name):
    report_list = Report.objects.filter(user_university=school_name).exclude(
        pre_etc__exact='').order_by('-user_duration')
    if not report_list:
        return render(request, 'forums/empty.html', {'school_name': school_name})

    update_date = Report.objects.filter(user_university=school_name).exclude(
        pre_etc__exact='').aggregate(Max('user_duration'))
    report = report_list[0]
    context = {
        'school_name': school_name,
        'pre_list': [],
